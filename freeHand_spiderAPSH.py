...
def setupParserOptions():
        ...
        parser.set_usage("%prog -u <untilted stack> -t <tilted stack> -m <model> -p <free_param> -c <ctf>")
	...
	parser.add_option("-t",dest="tilted",type="string",metavar="FILE",
        ...
        parser.add_option("-m",dest="model",type="string",metavar="FILE",
                help="3D model for alignment (Single SPIDER volume)")
...
def checkConflicts(params):
	...
	if not params['model']:
                ...
                sys.exit()
	spitest = params['model'][-4:]

	if spitest != '.spi':
		print "\nError: model needs to be a spider volume"
		sys.exit()
	if not params['param']:
...
def getMYAMI():
	myamipath = subprocess.Popen("env | grep MYAMI", shell=True, stdout=subprocess.PIPE).stdout.read().strip()
        ...
        if os.path.exists(myamipath):
                return myamipath
        print "appion was not found, make sure appion/2.2 is loaded"
        sys.exit()
...
def getCCP4Path():        
        ### get the openmpi directory        
        ccp4path = subprocess.Popen("env | grep CCP4=", shell=True, stdout=subprocess.PIPE).stdout.read().strip()        
...
def align(params,cwd):
	...
	#Prepare stack for EMAN2 refinement 
        print '\n'
        print 'Filtering & converting stack into SPIDER format'
        print '\n'
	...
	subprocess.Popen(cmd,shell=True).wait()
        ...
        #Run refinement
        print '\n'                
	print 'Running SPIDER alignment'                
	print '         Angular step = %s' %(ang)                
	print '         First ring, last ring = %s,%s' %(str(firstR),str(lastR))
	...
	searchR = (float(box)/2)-lastR-3
	searchR = int(searchR/float(ts))*float(ts)
	searchR = str(searchR)

	APSH('%s_prep' %(untilt[:-4]),model,ang,rad,box,firstR,lastR,ts,ray,mirror,searchR,tot)

def sort(paramout,tilt,ctf,num_mod,debug,tot,CC):
	...
	if int(num_mod) == 1:
		...
		o1 = open(c_o,'w')
		y_o = '%s_model00' %(paramout[:-4])
		y1 = open(y_o,'w')

       		text=open(text,'w')

		#Calc std devi and average CC
        	apsh = open(paramout,'r')
		avg = 0
		...
		apsh = open(paramout,'r')
                ...
                for line in apsh:
                        ...
                        if line[1] == ';':
                                continue
			std = std + (avg-float(l[12]))*(avg-float(l[12]))
		apsh.close()
		std = math.sqrt(std/float(tot))
			
		if debug is True:
			print 'Standard deviation is %f' %(std)

		lim = avg + (std)*(float(CC))
...
def angConv(paramout,num_mod,ctf,mag,model,tilt,debug):
	mod_count = 0
	...
	while mod_count < int(num_mod):
		...
		parm='%s_model%02d' %(paramout[:-4],mod_count)
		if debug is True:
			print 'parm = %s' %(parm)
		...
		for line in f:
			...
			sy =-(float(l[9]))
			model1 = float(l[6])
			out.write("%s 	%s	%s	%s	%s	%s\n"%(parmPSI,parmTHETA,parmPHI,sx,sy,model1))
		...
		cmd = 'e2proc2d.py --twod2threed %s_%02d.img %s_%02d.mrc' %(tilt[:-4],mod_count,tilt[:-4],mod_count)
		if debug is True:
			print cmd
		subprocess.Popen(cmd,shell=True).wait() 
...
def format_conv(params,cwd):
	...
	debug = params['debug']
	model = params['model']
	paramout = 'apsh_output.spi'

	#Sort particles based up membership to model(s)
	sort(paramout,tilt,ctf,num_mod,debug,tot,CC)
	
	#Convert formatting into fast free hand format
	angConv(paramout,num_mod,ctf,mag,model,tilt,debug)

	#Clean up
	mod = 0
	while mod < int(num_mod):
		
		cmd = 'rm %s_model%02d %s_model%02d_freeHand %s_model%02d.par %s_%02d.img %s_%02d.hed %s_%02d.txt' %(paramout[:-4],mod,paramout[:-4],mod,ctf[:-4],mod,tilt[:-4],mod,tilt[:-4],mod,tilt[:-4],mod)
		if debug is True:
			print cmd
		subprocess.Popen(cmd,shell=True).wait()
...
def fastFree_run(params,cwd,mod):
	...
	ctf = 'apsh_output_model00_free_format.par'
	...
	for line in ctfInfo:
		...
		if counter > 1:
			...
			if counter < tot:
			        if debug is True:
					print '          counter < tot - %f < %f' %(counter,tot)
...
def plotFH(params,ccp4_path,cwd):
	...
	while mod < num_mod:
		...
		while i <= int(num):
			...
			cmd = 'proc2d model%02d_plots_CC_v101_%02d.img model%02d_plots_CC_v101_merge.img rot=180' %(mod,i,mod)
			if debug is True:
				print cmd
			subprocess.Popen(cmd,shell=True).wait()
...
def APSH(stack,model,ang,radius,box,firstR,lastR,ts,ray,mirror,searchR,tot):
	...
	spicmd+="_9@******\n"
	spicmd+="1-[numang]\n"
	spicmd+="%s,%s,%s\n" %(searchR,searchR,str(ts))
	spicmd+="%s,%s,%s\n" %(str(firstR),str(lastR),str(ray))
	spicmd+="voea\n" 
	spicmd+="%s@******\n" %(stack)
	spicmd+="1-%s\n" %(str(tot))
	spicmd+="*\n"
...
if __name__ == "__main__":     
	...
	cwd = '/home/michael/Scripts_lab/FreeHand/lib'
        #if params['appion'] is True:
        #        format_ctf(params['ctf'])
	align(params,cwd)
	format_conv(params,cwd)
	fastFree(params,cwd)
	plotFH(params,ccp4,cwd)

	#Cleanup
	cmd = 'rm tmp.csh tmp2.csh totsumstack.exe apsh_output* model00_plots* z.plot %s_prep.* %s_00.mrc %s_000.mrc' %(params['untilted'][:-4],params['tilted'][:-4],params['model'][:-4])	
